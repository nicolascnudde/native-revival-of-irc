...
// Component
const NewChannelMessageForm = ({ channel }) => {
  ...
  const handleSubmit = async (message, { resetForm }) => {
    try {
      ...
      // Call the useStorage hook to upload the image and/or voice recording to firebase storage
      const storageImage = message.image ? await useStorage(message.image) : null;
      const storageRecording = message.recording ? await useStorage(message.recording, 'recordings') : null;

      const result = await addDoc(collection(db, 'channels', channel.id, 'messages'), {
          text: message.text,
...
// Export
export default NewChannelMessageForm;
